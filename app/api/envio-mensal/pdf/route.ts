...
<body>
  ...
  <div class="footer">
    ...
    if (!createResponse.ok) {
      return NextResponse.json({ error: 'Erro ao criar requisição de PDF' }, { status: 500 });
    }

    const { request_id } = await createResponse.json();
    if (!request_id) {
      return NextResponse.json({ error: 'Nenhum request_id retornado' }, { status: 500 });
    }
      ...
      if (statusResult?.status === 'SUCCESS' && statusResult?.result?.result) {
        ...
        return new NextResponse(pdfBuffer, {
          headers: {
            ...
            'Content-Disposition': `attachment; filename="relatorio_${customerName.replace(/[^a-zA-Z0-9]/g, '_')}.pdf"`,
          },
        });
